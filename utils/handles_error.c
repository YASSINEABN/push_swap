...
int	error_duplicate(l *a, int n) //Define a function that checks for duplicate input numbers in stack `a`
{
...
void	free_stack(l **stack) //Define a function to free a stack if there are errors
{
...
void	free_errors(l **a) //Define a function that, upon encountering a unique error, to free the stack and print an error message
{
	free_stack(a);
	printf("Error\n");
	exit(1);
